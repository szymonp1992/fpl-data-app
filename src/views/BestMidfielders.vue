  ...
  <div class="container mt-4">
    ...
    <div class="stats-table">
      ...
      >
        ...
        <tbody>
          <tr v-for="midfielder in sortedPlayers" :key="midfielder.id">
            <td>
              {{ midfielder.display_name }}
            </td>
            <td>{{ midfielder.team_name }}</td>
            <td>
              {{ midfielder.minutes }}
            </td>
            <td
            :style="{
                backgroundColor: colorCells('total_points', midfielder.total_points, 'desc'),
                border: borderForTopThree('total_points', midfielder.total_points, 'desc'),
              }">
              {{ midfielder.total_points }}
            </td>
            <td
              :style="{
                backgroundColor: colorCells('goals_scored', midfielder.goals_scored, 'desc'),
                border: borderForTopThree('goals_scored', midfielder.goals_scored, 'desc'),
              }"
            >
              {{ midfielder.goals_scored }}
            </td>
            <td
              :style="{
                backgroundColor: colorCells('assists', midfielder.assists, 'desc'),
                border: borderForTopThree('assists', midfielder.assists, 'desc'),
              }"
            >
              {{ midfielder.assists }}
            </td>
            <td
              :style="{
                display: currentFilter === 'per90Radio' ? 'none' : '',
                backgroundColor: colorCells('clean_sheets', midfielder.clean_sheets, 'desc'),
                border: borderForTopThree('clean_sheets', midfielder.clean_sheets, 'desc'),
              }"
            >
              {{ midfielder.clean_sheets }}
            </td>
            <td
              :style="{
                display: currentFilter === 'allSeasonRadio' ? 'none' : '',
                backgroundColor: colorCells('clean_sheets_per_90', midfielder.clean_sheets_per_90, 'desc'),
                border: borderForTopThree('clean_sheets_per_90', midfielder.clean_sheets_per_90, 'desc'),
              }"
            >
              {{ midfielder.clean_sheets_per_90 }}
            </td>
            <td
              :style="{
                display: currentFilter === 'per90Radio' ? 'none' : '',
                backgroundColor: colorCells('goals_conceded', midfielder.goals_conceded, 'asc'),
                border: borderForTopThree('goals_conceded', midfielder.goals_conceded, 'asc'),
              }"
            >
              {{ midfielder.goals_conceded }}
            </td>
            <td
              :style="{
                display: currentFilter === 'allSeasonRadio' ? 'none' : '',
                backgroundColor: colorCells('goals_conceded_per_90', midfielder.goals_conceded_per_90, 'asc'),
                border: borderForTopThree('goals_conceded_per_90', midfielder.goals_conceded_per_90, 'asc'),
              }"
            >
              {{ midfielder.goals_conceded_per_90 }}
            </td>
            <td
              :style="{
                display: currentFilter === 'per90Radio' ? 'none' : '',
                backgroundColor: colorCells('expected_goals_conceded', midfielder.expected_goals_conceded, 'asc'),
                border: borderForTopThree('expected_goals_conceded', midfielder.expected_goals_conceded, 'asc'),
              }"
            >
              {{ midfielder.expected_goals_conceded }}
            </td>
            <td
              :style="{
                display: currentFilter === 'allSeasonRadio' ? 'none' : '',
                backgroundColor: colorCells('expected_goals_conceded_per_90', midfielder.expected_goals_conceded_per_90, 'asc'),
                border: borderForTopThree('expected_goals_conceded_per_90', midfielder.expected_goals_conceded_per_90, 'asc'),
              }"
            >
              {{ midfielder.expected_goals_conceded_per_90 }}
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  setup() {
    const midfieldersList = ref([]);
    const midfieldersListFiltered = ref([]);
    ...
    const sortedPlayers = computed(() => {
      return midfieldersListFiltered.value.sort((a, b) => {
        let modifier = 1;
    ...
    async function loadmidfieldersData() {
      ...
      // Filtering only midfielders from all players
      const midfielders = allPlayers.filter((player) => {
        return player.element_type === 3 && player.status !== "u";
      });
      // Adding every midfielder his team name based on teamsCodes
      midfielders.forEach((midfielder) => {
        // Getting goalie team code to match it with codes in teamsCodes array
        const gkTeamCode = midfielder.team_code;
        // Finding a team that has ID that matches team_code in midfielder object
        const matchedTeam = teamsCodes.find((team) => {
          return team.teamCode == gkTeamCode;
        });
        // Appending team_name property to every midfielder object
        midfielder.team_name = matchedTeam.teamName;
        let midfielderDisplayName
        if (midfielder.second_name !== midfielder.web_name) {
          midfielderDisplayName = midfielder.web_name
        } else {
          midfielderDisplayName = `${midfielder.first_name} ${midfielder.second_name}`
        }
        midfielder.display_name = midfielderDisplayName
      });
      midfieldersList.value = midfielders;
      midfieldersListFiltered.value = midfielders;
    }
    ...
    function filterByMinMinutes(event) {
      ...
      if (!event.target.value) {
        midfieldersListFiltered.value = midfieldersList.value;
      } else {
        midfieldersListFiltered.value = midfieldersList.value.filter(
          (player) => {
